	...
	@find $(BUILD_DIR)/assets -type f \
		\( -name '*.template' -o -name '*.yml' -o -name '*.yaml' \
			...
			-o -name '*.service' \) -exec sed -i \
				...
				-e "s|__CONSOLE_CPU_GOVERNOR_PERFORMANCE__|$(CONSOLE_CPU_GOVERNOR_PERFORMANCE)|g" \
				-e "s|__CONSOLE_IRQBALANCE_ENABLED__|$(CONSOLE_IRQBALANCE_ENABLED)|g" \
				-e "s|__GAMEMODE_ENABLED__|$(GAMEMODE_ENABLED)|g" \
				-e "s|__GAMEMODE_IOPRIO__|$(GAMEMODE_IOPRIO)|g" \
				-e "s|__GAMEMODE_RENICE__|$(GAMEMODE_RENICE)|g" \
				-e "s|__GAMEMODE_SOFTREALTIME__|$(GAMEMODE_SOFTREALTIME)|g" \
				-e "s|__GAMEMODE_INHIBIT_SCREENSAVER__|$(GAMEMODE_INHIBIT_SCREENSAVER)|g" \
				-e "s|__CONSOLE_NVIDIA_POWERMIZER_MAX_PERF__|$(CONSOLE_NVIDIA_POWERMIZER_MAX_PERF)|g" \
				-e "s|__CONSOLE_NVIDIA_ENABLE_MSI__|$(CONSOLE_NVIDIA_ENABLE_MSI)|g" \
				-e "s|__CONSOLE_NVIDIA_XORG_TUNING__|$(CONSOLE_NVIDIA_XORG_TUNING)|g" \
				-e "s|__CONSOLE_NVIDIA_COOLBITS__|$(CONSOLE_NVIDIA_COOLBITS)|g" \
				-e "s|__CONSOLE_SYSCTL_TUNING__|$(CONSOLE_SYSCTL_TUNING)|g" \
				-e "s|__CONSOLE_SYSCTL_VM_MAX_MAP_COUNT__|$(CONSOLE_SYSCTL_VM_MAX_MAP_COUNT)|g" \
				-e "s|__CONSOLE_SYSCTL_VM_SWAPPINESS__|$(CONSOLE_SYSCTL_VM_SWAPPINESS)|g" \
				-e "s|__CONSOLE_IO_SCHEDULER_TUNING__|$(CONSOLE_IO_SCHEDULER_TUNING)|g" \
				-e "s|__CONSOLE_IO_SCHEDULER_NVME__|$(CONSOLE_IO_SCHEDULER_NVME)|g" \
				-e "s|__CONSOLE_IO_SCHEDULER_SSD__|$(CONSOLE_IO_SCHEDULER_SSD)|g" \
				-e "s|__CONSOLE_IO_SCHEDULER_HDD__|$(CONSOLE_IO_SCHEDULER_HDD)|g" \
				-e "s|__MANGOHUD_ENABLED__|$(MANGOHUD_ENABLED)|g" \
				{} +; \
