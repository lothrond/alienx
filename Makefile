	...
	@find $(BUILD_DIR)/assets -type f \
		\( -name '*.template' -o -name '*.yml' -o -name '*.yaml' \
			...
			-o -name '*.service' \) -exec sed -i \
				...
				-e "s|__CPU_GOVERNOR_PERFORMANCE__|$(CPU_GOVERNOR_PERFORMANCE)|g" \
				-e "s|__IRQBALANCE_ENABLED__|$(IRQBALANCE_ENABLED)|g" \
				...
				-e "s|__NVIDIA_POWERMIZER_MAX_PERF__|$(NVIDIA_POWERMIZER_MAX_PERF)|g" \
				-e "s|__NVIDIA_ENABLE_MSI__|$(NVIDIA_ENABLE_MSI)|g" \
				-e "s|__NVIDIA_XORG_TUNING__|$(NVIDIA_XORG_TUNING)|g" \
				-e "s|__NVIDIA_COOLBITS__|$(NVIDIA_COOLBITS)|g" \
				-e "s|__SYSCTL_TUNING__|$(SYSCTL_TUNING)|g" \
				-e "s|__SYSCTL_VM_MAX_MAP_COUNT__|$(SYSCTL_VM_MAX_MAP_COUNT)|g" \
				-e "s|__SYSCTL_VM_SWAPPINESS__|$(SYSCTL_VM_SWAPPINESS)|g" \
				-e "s|__IO_SCHEDULER_TUNING__|$(IO_SCHEDULER_TUNING)|g" \
				-e "s|__IO_SCHEDULER_NVME__|$(IO_SCHEDULER_NVME)|g" \
				-e "s|__IO_SCHEDULER_SSD__|$(IO_SCHEDULER_SSD)|g" \
				-e "s|__IO_SCHEDULER_HDD__|$(IO_SCHEDULER_HDD)|g" \
